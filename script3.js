...
function listeVisning(){
  ...
  i++;
  tjohei.innerHTML += "<li>" + "toalett " + i +" ved " + "<a id='toalettlenke' onclick='klikkern(" + (i) + ")'>" + toalett.place + "</a></li>" + ("</br>");
});
}
...
//Under kommer tekst-søkefunksjon kode SE BORT I FRA DETTE
    ...
    if(resultat[0] = "mann" || "herre" || "gutt"){
      for(var i = 0; i < toaletter.length; i++){
        if(toaletter[i].herre != "NULL"){
          var tjohei = document.getElementById("smør");
          tjohei.innerHTML += "<li>" + "toalett " + (i+1) +" ved " + "<a id='toalettlenke' onclick='klikkern(" + (i+1) + ")'>" + toaletter[i].place + "</li>" +  ("</br>");
...
//Checker /unchecker alle checkboxes - Odin
...
//  Logger ut og returnerer tiden. Filter for alternativ tidsvisning. -Odin
function getTime(){
...
//Finner ukedagstallet og setter verdi av uke/lør/søn. Brukes i tidssøk. -Odin
function dagen() {
    var d = new Date();
    var n = d.getDay()
    var x;
...
Benytter seg av tidsFiltrering for tids-søk. -Odin
*/
function sjekk(element){
  ...
  if(document.getElementById('handicap').checked){
    ...
    for (var i = 0; i < listen.length; i++) {
      innholdet.innerHTML += "<li>" + "toalett " + (i+1) +" ved " + "<a id='toalettlenke' onclick='klikkern(" + (i+1) + ")'>" + listen[i].place + "</li>" + ("</br>");
    }
    reduserListe(listen);
...
function tekstSearch(){
  ...
  for (var i = 0; i < liste.length; i++) {
            innholdet.innerHTML += "<li>" + "toalett " + (i+1) +" ved " + "<a id='toalettlenke' onclick='klikkern(" + (i+1) + ")'>" + liste[i].place + "</li>" +("</br>");
  }
  reduserListe(liste);
}

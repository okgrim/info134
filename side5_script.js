...
{
  i++;
  var liste = document.getElementById("toalettListe");
  liste.innerHTML += i + ") Sted: " + x.place + ("</br>");
});

toaletter.forEach(function(y)
{
  console.log(y.place);
});

function initMap() {
        var bergen = {lat: 60.397557, lng: 5.307858};
    ...
    toaletter.forEach(function(toalett){
      var lat1 = Number(toalett.latitude);
      var lng1 = Number(toalett.longitude);
...
var marker = new google.maps.Marker({
  position: {lat: 60.388076, lng: 5.334937},
  map: map,
  title: 'Toalett 1'
});

// for(var i = 0; i<toaletter.length; i++){
//   var loc = toaletter[i].latitude, toaletter[i].longitude;
//   marker = new google.maps.Marker({
//     position: loc,
//     map: map,
//     title: "Toalett " + toaletter[i].id
//   });
// }
